...
def _find_downstream(
  ...
  # We mark all consts as used since they could be used by the fusion.
  num_constvars = len(jaxpr.constvars)
  jaxpr_closure_converted = pe.convert_constvars_jaxpr(jaxpr)
  in_used = [True] * num_constvars + list(in_used)
  _, _, out_used, *_ = pe.partial_eval_jaxpr_custom(
      jaxpr_closure_converted,
      in_unknowns=in_used,
      in_inst=in_used,
...
def _construct_output_fusions(
    ...
    output_fusion_mask,  # Pytree defining output groups
):
  ...
  partial_flat = jax.tree.structure(output_fusion_mask).flatten_up_to(
  ...
  return (
      tree_util.tree_unflatten(
          tree_util.tree_structure(output_fusion_mask), output_fusions
      ),
      output_permutation,
  )
...
def fuse_jaxpr(
  ...
  output_fusions, output_permutation = _construct_output_fusions(
      ...
      fusion_eqn.params["output_fusion_mask"],
  )
  ...
  assert len(permuted_out) == len(jaxpr.outvars), (len(permuted_out), len(jaxpr.outvars))
  return permuted_out
