...
def _handle_transforms(
  ...
  def _bubble_up(untransform_fn, data):
    ...
    return data, list(reversed(new_transforms_rev))
  ...
  for t in transforms:
    match t:
      case indexing.NDIndexer():
        ...
        indices, new_transforms = _bubble_up(
            lambda t, idxs: t.untransform_index(idxs), indices
        )
        transformed_ref = mgpu.memref_slice(transformed_ref, indices)
      case gpu_core.TransposeRef(perm):
        if not handle_transposes:
          new_transforms.append(t)
          continue
        perm, new_transforms = _bubble_up(lambda t, p: t.untransform_transpose(p),
                                          perm)
        transformed_ref = mgpu.memref_transpose(transformed_ref, perm)
      case RefReshaper(dtype=dtype, shape=shape):
        if not handle_reshapes:
          new_transforms.append(t)
          continue

        shape, new_transforms = _bubble_up(
            lambda t, p: t.untransform_reshape(dtype, p),
            shape)
